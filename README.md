...
**A**utomated **R**oot **C**ause **A**nalysis & **D**iagnostic **I**ncident **A**ssistant —
the acronym is hidden in the middle of the word, and the console's logotype shows it:
A*rca*dia.

The mark is an arch. An arch is held up by its keystone, the one piece that, once
found, holds everything else in place — which is what a root cause is. It also
opens with the arc the name does, and reduces to a clean arc at favicon size.

> **Note on naming.** Arcadia is the product name. It is deliberately *not* the
> infrastructure name: every AWS resource keeps the `app-b9dac5ac-bc8fbf47-`
> prefix that `CLAUDE.md` fixes, and the CloudFormation stack is still
> `app-b9dac5ac-bc8fbf47-triage`. Renaming the stack would not rename anything —
> it would build a second one alongside the live deployment and hand it a new
> CloudFront domain. The same reasoning keeps `frontend/lib/triage.mjs` and the
> `triage:lib` markers as they are: they are internal identifiers, and churning
> them buys nothing.
